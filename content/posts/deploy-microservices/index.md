...
tags:
  ['microservices', 'Traefik', 'DevOps', 'software', 'web', 'Docker', 'YAML']
---

I've been running this blog on a bare-metal server using [Nginx](https://www.nginx.com/) as a reverse proxy to basically serve the static files generated by Gatsby. But now I want to deploy multiple docker services on my [VPS](https://en.wikipedia.org/wiki/Virtual_private_server).

To help with that endeavour, I decided to try using Traefik, which is an edge router that can act as a reverse proxy and route multiple services on my personal domain. Unfortunately, my brain is not big enough to understand all these new concepts. So, I decided to document it.

## Goals

First, I'd like to plan out what the goals are so I can figure out how to get there:
...
Essentially, what my `Dockerfile` does is the following:
...
Since I wrote this post, I have hosted several different services on my domain. So I thought it would be a good idea to come and update a list of all services on my domain every so often.
